#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Evaluate the target model: gated + NSRE-weighted + FullSizeCNN + Mish."""
...
OUT = DATA / "target_model_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 60
BATCH_SIZE = 64
...
class GatedBranchMish(nn.Module):
    ...
    def forward(self, x):
        x = self.conv(x); x = self.bn(x); x = F.mish(x)
        x = F.adaptive_avg_pool2d(x, (1, 1)).flatten(1)
        return x * torch.sigmoid(self.gate(x))


class TargetMulti(nn.Module):
    def __init__(self, out_dim):
        super().__init__()
        self.mrna = GatedBranchMish(SIZES["mRNA"], CHANNELS)
        self.cnv = GatedBranchMish(SIZES["CNV"], CHANNELS)
        self.mirna = GatedBranchMish(SIZES["miRNA"], CHANNELS)
        self.head = nn.Sequential(
            nn.Linear(CHANNELS * 3, 128), nn.Mish(inplace=True), nn.Dropout(0.3),
            nn.Linear(128, 64), nn.Mish(inplace=True), nn.Dropout(0.3),
...
class TargetSingle(nn.Module):
    def __init__(self, size, out_dim):
        super().__init__()
        self.branch = GatedBranchMish(size, CHANNELS)
        self.head = nn.Sequential(nn.Linear(CHANNELS, 64), nn.Mish(inplace=True), nn.Dropout(0.3), nn.Linear(64, out_dim))
    def forward(self, x):
...
def eval_pam50_single(omics):
    ...
    for tr, te in cv.split(np.zeros(len(cases)), y):
        ...
        model = TargetSingle(size, len(enc.classes_)); torch_seed(); opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4); crit = nn.CrossEntropyLoss(); yt = torch.tensor(y[tr], dtype=torch.long)
...
def eval_pam50_multi():
    ...
    for tr, te in cv.split(np.zeros(len(common)), y):
        ...
        model = TargetMulti(len(enc.classes_)); torch_seed(); opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4); crit = nn.CrossEntropyLoss(); yt = torch.tensor(y[tr], dtype=torch.long)
...
def eval_os_single(omics):
    ...
    for tr, te in cv.split(np.zeros(len(cases)), y_event):
        ...
        Xtr_img = torch.tensor(make_weighted(Xtr, size, order, scores), dtype=torch.float32)
        Xte_img = torch.tensor(make_weighted(Xte, size, order, scores), dtype=torch.float32)
        model = TargetSingle(size, 1); torch_seed(); opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4); crit = nn.BCEWithLogitsLoss(); yt = torch.tensor(y_event[tr], dtype=torch.float32).view(-1,1)
        n = Xtr_img.shape[0]; model.train()
        for _ in range(EPOCHS):
            perm = torch.randperm(n)
            for i in range(0, n, BATCH_SIZE):
        ...
        model = TargetSingle(size, 1); torch_seed(); opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4); time_t = torch.tensor(y_time[tr], dtype=torch.float32); event_t = torch.tensor(y_event[tr], dtype=torch.float32)
        n = Xtr_img.shape[0]; model.train()
        for _ in range(EPOCHS):
            perm = torch.randperm(n)
            for i in range(0, n, BATCH_SIZE):
...
def eval_os_multi():
    ...
    for tr, te in cv.split(np.zeros(len(common)), y_event):
        ...
        for block, omics in zip(X_blocks, OMICS):
            ...
            train_imgs.append(torch.tensor(make_weighted(Xtr, SIZES[omics], order, scores), dtype=torch.float32))
            test_imgs.append(torch.tensor(make_weighted(Xte, SIZES[omics], order, scores), dtype=torch.float32))
        model = TargetMulti(1); torch_seed(); opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4); crit = nn.BCEWithLogitsLoss(); yt = torch.tensor(y_event[tr], dtype=torch.float32).view(-1,1)
        n = train_imgs[0].shape[0]; model.train()
        for _ in range(EPOCHS):
            perm = torch.randperm(n)
            for i in range(0, n, BATCH_SIZE):
        ...
        aucs.append(roc_auc_score(y_event[te], prob))
        model = TargetMulti(1); torch_seed(); opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4); time_t = torch.tensor(y_time[tr], dtype=torch.float32); event_t = torch.tensor(y_event[tr], dtype=torch.float32)
        n = train_imgs[0].shape[0]; model.train()
        for _ in range(EPOCHS):
            perm = torch.randperm(n)
            for i in range(0, n, BATCH_SIZE):
...
def main() -> None:
    ...
    for omics in OMICS:
        ...
        rows.append({"model": f"Target_{omics}", "task": "PAM50", "metric": "accuracy", "mean": round(acc,4), "std": round(acc_std,4)})
        rows.append({"model": f"Target_{omics}", "task": "PAM50", "metric": "macro_f1", "mean": round(f1,4), "std": round(f1_std,4)})
        auc, auc_std, ci, ci_std = eval_os_single(omics)
        rows.append({"model": f"Target_{omics}", "task": "OS", "metric": "roc_auc", "mean": round(auc,4), "std": round(auc_std,4)})
        rows.append({"model": f"Target_{omics}", "task": "OS", "metric": "c_index", "mean": round(ci,4), "std": round(ci_std,4)})
        print(f"{omics} done")
    acc, acc_std, f1, f1_std = eval_pam50_multi()
    rows.append({"model": "Target_Multi", "task": "PAM50", "metric": "accuracy", "mean": round(acc,4), "std": round(acc_std,4)})
    rows.append({"model": "Target_Multi", "task": "PAM50", "metric": "macro_f1", "mean": round(f1,4), "std": round(f1_std,4)})
    auc, auc_std, ci, ci_std = eval_os_multi()
    rows.append({"model": "Target_Multi", "task": "OS", "metric": "roc_auc", "mean": round(auc,4), "std": round(auc_std,4)})
    rows.append({"model": "Target_Multi", "task": "OS", "metric": "c_index", "mean": round(ci,4), "std": round(ci_std,4)})
