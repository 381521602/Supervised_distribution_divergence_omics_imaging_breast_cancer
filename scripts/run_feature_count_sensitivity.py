#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Quick sensitivity check for the number of retained features."""
...
OUT = DATA / "feature_count_sensitivity.tsv"
MATRICES = {
...
def pam50_cv(omics: str, X, y, k: int) -> dict:
    model = Pipeline(
        [
...
def cox_cv(omics: str, X, y_time, y_event, k: int) -> dict:
    cv = StratifiedKFold(n_splits=K_FOLDS, shuffle=True, random_state=RANDOM_STATE)
...
def main() -> None:
    ...
    for omics, path in MATRICES.items():
        ...
        for k in [50, 100, 200, 300, 500]:
            rows.append(pam50_cv(omics, X, y, k))
        ...
        if omics == "mRNA":
            ...
            for k in [20, 50, 100, 200]:
                rows.append(cox_cv(omics, X, y_time, y_event, k))
