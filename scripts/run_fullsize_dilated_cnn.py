#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Full-size convolution + dilated convolution multi-stream CNN."""
...
OUT = DATA / "fullsize_dilated_cnn_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 80
BATCH_SIZE = 64
...
DILATIONS = {"mRNA": [2, 3], "CNV": [2], "miRNA": [2, 3]}
CHANNELS = 32


class FullSizeDilatedBranch(nn.Module):
    def __init__(self, size: int, dilations: list[int], channels: int):
        super().__init__()
        self.full = nn.Conv2d(1, channels, kernel_size=(size, size), padding=0)
        self.full_bn = nn.BatchNorm2d(channels)
        self.dilated = nn.ModuleList(
            [nn.Conv2d(1, channels, kernel_size=3, padding=d, dilation=d) for d in dilations]
        )
        self.dilated_bns = nn.ModuleList([nn.BatchNorm2d(channels) for _ in dilations])
    ...
    def forward(self, x):
        ...
        y = self.full_bn(y)
        y = F.relu(y)
        outs.append(F.adaptive_avg_pool2d(y, (1, 1)).flatten(1))
        for conv, bn in zip(self.dilated, self.dilated_bns):
            z = conv(x)
            z = bn(z)
            z = F.relu(z)
            outs.append(F.adaptive_avg_pool2d(z, (1, 1)).flatten(1))
        return torch.cat(outs, dim=1)


class FullSizeDilatedNet(nn.Module):
    def __init__(self, out_dim: int):
        super().__init__()
        self.mrna = FullSizeDilatedBranch(SIZES["mRNA"], DILATIONS["mRNA"], CHANNELS)
        self.cnv = FullSizeDilatedBranch(SIZES["CNV"], DILATIONS["CNV"], CHANNELS)
        self.mirna = FullSizeDilatedBranch(SIZES["miRNA"], DILATIONS["miRNA"], CHANNELS)
        mrna_dim = CHANNELS * (1 + len(DILATIONS["mRNA"]))
        cnv_dim = CHANNELS * (1 + len(DILATIONS["CNV"]))
        mirna_dim = CHANNELS * (1 + len(DILATIONS["miRNA"]))
        dim = mrna_dim + cnv_dim + mirna_dim
...
def run_fold(train_imgs, test_imgs, y_train, y_test, out_dim):
    torch_seed()
    model = FullSizeDilatedNet(out_dim).to(DEVICE)
...
def main() -> None:
    ...
    rows = [
        {"model": "FullSizeConv_Dilated", "metric": "accuracy", "value": round(float(np.mean(accs)), 4)},
        {"model": "FullSizeConv_Dilated", "metric": "macro_f1", "value": round(float(np.mean(f1s)), 4)},
    ]
