#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Method 4: low-rank bilinear fusion."""
...
OUT = DATA / "advanced_method4_lowrank_bilinear_results.tsv"
...
class LowRankFusion(nn.Module):
    def __init__(self, dims, d=32, rank=RANK, out_dim=1):
        super().__init__()
        self.proj = nn.ModuleDict({o: nn.Linear(dims[o], d) for o in OMICS})
        self.U = nn.Linear(d, rank, bias=False)
        self.V = nn.Linear(d, rank, bias=False)
        self.W = nn.Linear(d, rank, bias=False)
        self.head = nn.Linear(rank, out_dim)

    def forward(self, xs):
        h = {o: self.proj[o](xs[o]) for o in OMICS}
        z = self.U(h["mRNA"]) * self.V(h["CNV"]) * self.W(h["miRNA"])
        return self.head(z)
...
def train(Xtr, ytr, Xte, dims, binary):
    ...
    model = LowRankFusion(dims, out_dim=1 if binary else 4)
