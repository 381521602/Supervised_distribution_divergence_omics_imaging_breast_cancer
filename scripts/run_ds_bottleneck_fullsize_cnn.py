#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Bottleneck + depthwise-separable full-size convolution."""
...
OUT = DATA / "ds_bottleneck_fullsize_cnn_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 60
BATCH_SIZE = 64
...
class DSBottleneckBranch(nn.Module):
    def __init__(self, size, bottleneck, channels):
        super().__init__()
        self.reduce = nn.Conv2d(1, bottleneck, 1)
        self.depthwise = nn.Conv2d(bottleneck, bottleneck, kernel_size=(size, size), groups=bottleneck)
...
class DSBottleneckMultiStream(nn.Module):
    def __init__(self, out_dim):
        super().__init__()
        self.mrna = DSBottleneckBranch(SIZES["mRNA"], BOTTLENECK, CHANNELS)
        self.cnv = DSBottleneckBranch(SIZES["CNV"], BOTTLENECK, CHANNELS)
        self.mirna = DSBottleneckBranch(SIZES["miRNA"], BOTTLENECK, CHANNELS)
...
def main() -> None:
    ...
    rows = [
        {"model": "DS_Bottleneck_FullSizeCNN", "metric": "accuracy", "value": round(float(np.mean(accs)), 4)},
        {"model": "DS_Bottleneck_FullSizeCNN", "metric": "macro_f1", "value": round(float(np.mean(f1s)), 4)},
    ]
