...
def main():
    ...
    add_body(doc,"第一步，对每个特征计算 JSD 分数。JSD 通过比较正负类样本特征值概率分布的差异来衡量该特征的判别能力，分数越高表示该特征在不同类别间的分布差异越大。")
    add_body(doc,"第二步，按照 JSD 分数从高到低对所有特征进行排序。排序越靠前的特征，判别能力越强。")
    add_body(doc,"第三步，采用中心向外的螺旋顺序将排序后的特征逐个填入方形网格。JSD 分数最高的特征被放置在图像中心，随着排序位置下降，特征逐渐向外围扩展。这种排列方式使最重要的特征集中在图像中心附近，有利于卷积运算重点关注核心区域，同时降低边界效应的影响。")
