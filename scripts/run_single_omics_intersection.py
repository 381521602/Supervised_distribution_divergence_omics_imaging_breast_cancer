#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Single-omics baselines on the same sample intersection as the fusion."""
...
OUT = DATA / "single_omics_intersection_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
...
def intersection(task: str):
    matrices = [load_matrix(FEAT_DIR / f"{omics}_{task}_matrix.tsv") for omics in ["mRNA", "CNV", "miRNA"]]
    common = matrices[0].index
    for m in matrices[1:]:
        common = common.intersection(m.index)
    return list(common), matrices
...
def cv_classification(omics: str, X, y) -> list[dict]:
    ...
    for name, clf in [
        ...
        rows.append({"omics": omics, "task": "PAM50_4class", "model": name, "metric": "accuracy", "value": round(float(np.mean(scores["test_accuracy"])), 4)})
        rows.append({"omics": omics, "task": "PAM50_4class", "model": name, "metric": "macro_f1", "value": round(float(np.mean(scores["test_f1_macro"])), 4)})
    return rows
...
def main() -> None:
    ...
    for task in ["PAM50_4class", "OS"]:
        common, matrices = intersection(task)
        for omics, matrix in zip(["mRNA", "CNV", "miRNA"], matrices):
