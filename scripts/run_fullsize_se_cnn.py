#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Full-size multi-stream CNN with Squeeze-and-Excitation channel attention."""
...
OUT = DATA / "fullsize_se_cnn_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 80
BATCH_SIZE = 64
...
class FullSizeSENet(nn.Module):
    def __init__(self, out_dim: int):
        super().__init__()
        self.mrna = SEConvBranch(SIZES["mRNA"], CHANNELS)
        self.cnv = SEConvBranch(SIZES["CNV"], CHANNELS)
        self.mirna = SEConvBranch(SIZES["miRNA"], CHANNELS)
...
def run_fold(train_imgs, test_imgs, y_train, y_test, out_dim):
    torch_seed()
    model = FullSizeSENet(out_dim).to(DEVICE)
...
def main() -> None:
    ...
    rows = [
        {"model": "FullSizeConv_SE", "metric": "accuracy", "value": round(float(np.mean(accs)), 4)},
        {"model": "FullSizeConv_SE", "metric": "macro_f1", "value": round(float(np.mean(f1s)), 4)},
    ]
