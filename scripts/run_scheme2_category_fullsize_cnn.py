#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""方案2单独处理：方案2表达图 + 基因类别图谱双分支 FullSizeCNN。"""
...
OUT = ROOT / "data/scheme2_category_fullsize_cnn_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 30
BATCH_SIZE = 64
...
def image_variants(task):
    ...
    gray = np.load(reorder / "images.npy")
    cat_grid = np.load(reorder / "category_grid.npy")
    cat = onehot_grid(cat_grid, gray.shape[0])
    return {
        "scheme2_single": [gray],
        "scheme2_category_two_branch": [gray, cat],
    }
