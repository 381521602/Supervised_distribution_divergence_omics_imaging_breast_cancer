...
def weighted_images(imgs, grid):
    return imgs * grid[np.newaxis,np.newaxis,:,:]
...
def eval_pam50():
    ...
    grid=weight_map("PAM50","mRNA",20); imgs=weighted_images(imgs,grid)
...
def eval_survival():
    ...
    grid=weight_map("Survival","mRNA",15); imgs=weighted_images(imgs,grid)
    cv=StratifiedKFold(n_splits=K_FOLDS,shuffle=True,random_state=RANDOM_STATE)
    aucs,cis=[],[]
