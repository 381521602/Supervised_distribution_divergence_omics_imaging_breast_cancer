#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""FullSizeCNN with grouped convolution branches."""
...
OUT = DATA / "grouped_fullsize_cnn_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 60
BATCH_SIZE = 64
...
class GroupedBranch(nn.Module):
    def __init__(self, size, channels, groups):
        super().__init__()
        self.expand = nn.Conv2d(1, channels, 1)
        self.grouped = nn.Conv2d(channels, channels, kernel_size=(size, size), groups=groups)
...
class GroupedMultiStream(nn.Module):
    def __init__(self, out_dim):
        super().__init__()
        self.mrna = GroupedBranch(SIZES["mRNA"], CHANNELS, GROUPS)
        self.cnv = GroupedBranch(SIZES["CNV"], CHANNELS, GROUPS)
        self.mirna = GroupedBranch(SIZES["miRNA"], CHANNELS, GROUPS)
...
def main() -> None:
    ...
    for tr, te in cv.split(np.zeros(len(common)), y):
        ...
        model = GroupedMultiStream(len(enc.classes_)); torch_seed(); opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4); crit = nn.CrossEntropyLoss(); yt = torch.tensor(y[tr], dtype=torch.long)
    ...
    rows = [
        {"model": "GroupedConv_FullSizeCNN", "metric": "accuracy", "value": round(float(np.mean(accs)), 4)},
        {"model": "GroupedConv_FullSizeCNN", "metric": "macro_f1", "value": round(float(np.mean(f1s)), 4)},
    ]
