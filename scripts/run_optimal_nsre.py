#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Test an optimized NSRE-based feature-selection pipeline.

Classification (PAM50):
  L1 coarse filter -> one-vs-rest NSRE fine filter -> LogisticRegression/RandomForest

Survival:
  L1 coarse filter -> binary NSRE fine filter -> LogisticRegression / CoxPH
"""
...
from run_entropy_feature_selection import nsre_between_histograms  # noqa: E402


DATA = ROOT / "data"
LABELS = DATA / "brca_labels_modeling_ready.tsv"
OUT = DATA / "optimal_nsre_results.tsv"
MATRICES = {
...
class OvRNSRESelector(BaseEstimator, TransformerMixin):
    """One-vs-rest NSRE: score a feature by max separation against each class."""
...
def build_classifier(selector_kind: str):
    ...
    return Pipeline(
        [
            ("variance", variance),
            ("coarse", coarse),
            ("ovr_nsre", OvRNSRESelector(k=200)),
...
def main() -> None:
    ...
    for omics, path in MATRICES.items():
        ...
        for method, clf_name, clf in [
            ("L1_OvRNSRE", "LogisticRegression", LogisticRegression(max_iter=2000, random_state=RANDOM_STATE)),
            ("L1_OvRNSRE", "RandomForest", RandomForestClassifier(n_estimators=300, random_state=RANDOM_STATE, n_jobs=-1)),
            ("FClassif_OvRNSRE", "LogisticRegression", LogisticRegression(max_iter=2000, random_state=RANDOM_STATE)),
        ]:
            try:
