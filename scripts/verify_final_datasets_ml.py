#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Verify final datasets with ML algorithms and 5-fold CV."""
...
OUT = ROOT / "data" / "final_datasets_ml_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 30
BATCH_SIZE = 64
...
def train_mlp(Xtr, ytr, Xte, yte, out_dim):
...
def eval_pam50():
    ...
    for omics in OMICS:
        ...
        for name, clf in classifiers().items():
            ...
            rows.append({"task":"PAM50","omics":omics,"model":name,"metric":"accuracy","mean":round(float(np.mean(accs)),4),"std":round(float(np.std(accs)),4)})
            rows.append({"task":"PAM50","omics":omics,"model":name,"metric":"macro_f1","mean":round(float(np.mean(f1s)),4),"std":round(float(np.std(f1s)),4)})
        accs,f1s=[],[]
        for tr,te in cv.split(np.zeros(len(y)),y):
            scaler=StandardScaler().fit(X[tr]); Xtr=scaler.transform(X[tr]); Xte=scaler.transform(X[te])
            pred=train_mlp(Xtr,y[tr],Xte,y[te],len(enc.classes_))
            accs.append(accuracy_score(y[te],pred)); f1s.append(f1_score(y[te],pred,average="macro"))
        rows.append({"task":"PAM50","omics":omics,"model":"MLP","metric":"accuracy","mean":round(float(np.mean(accs)),4),"std":round(float(np.std(accs)),4)})
        rows.append({"task":"PAM50","omics":omics,"model":"MLP","metric":"macro_f1","mean":round(float(np.mean(f1s)),4),"std":round(float(np.std(f1s)),4)})
    return rows
...
def eval_survival():
    ...
    for omics in OMICS:
        ...
        for name, clf in classifiers().items():
            ...
            rows.append({"task":"Survival","omics":omics,"model":name,"metric":"roc_auc","mean":round(float(np.mean(aucs)),4),"std":round(float(np.std(aucs)),4)})
            rows.append({"task":"Survival","omics":omics,"model":name,"metric":"c_index","mean":round(float(np.mean(cis)),4),"std":round(float(np.std(cis)),4)})
        cis=[]; cox_aucs=[]
        for tr,te in cv.split(np.zeros(len(y_event)),y_event):
            scaler=StandardScaler().fit(X[tr]); Xtr=scaler.transform(X[tr]); Xte=scaler.transform(X[te])
            ...
            cox_aucs.append(roc_auc_score(y_event[te], hazard))
        rows.append({"task":"Survival","omics":omics,"model":"CoxPH","metric":"c_index","mean":round(float(np.mean(cis)),4),"std":round(float(np.std(cis)),4)})
        rows.append({"task":"Survival","omics":omics,"model":"CoxPH","metric":"roc_auc","mean":round(float(np.mean(cox_aucs)),4),"std":round(float(np.std(cox_aucs)),4)})
    return rows
