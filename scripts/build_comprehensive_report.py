...
def main():
    ...
    add_body(doc,"基于 TCGA-BRCA 的 mRNA、CNV、miRNA 数据，采用最优特征方法提取特征，并保留特征名用于可解释性分析。PAM50 分型数据集：mRNA 使用 V_L1 保留 400 特征，CNV 使用 V_L1_F 保留 50 特征，miRNA 使用 V_F_NSRE 保留 600 特征。生存预测数据集：mRNA 使用 V_NSRE_F 保留 200 特征，CNV 使用 V_L1_NSRE 保留 150 特征，miRNA 使用 V_L1_NSRE 保留 600 特征。所有数据集均记录样本 ID、标签和特征名。")
    add_table(doc, ["任务","组学","特征方法","特征数","样本数"], [
        ["PAM50","mRNA","V_L1","400","833"],
        ["PAM50","CNV","V_L1_F","50","818"],
        ["PAM50","miRNA","V_F_NSRE","600","497"],
        ["生存预测","mRNA","V_NSRE_F","200","1073"],
        ["生存预测","CNV","V_L1_NSRE","150","1057"],
        ["生存预测","miRNA","V_L1_NSRE","600","739"],
