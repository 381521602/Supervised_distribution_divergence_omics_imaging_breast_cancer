#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Gated FullSizeCNN for survival prediction."""
...
OUT = DATA / "gated_fullsize_cnn_survival_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 60
BATCH_SIZE = 64
...
def run_multi(matrices, labels):
    ...
    for tr, te in cv.split(np.zeros(len(common)), y_event):
        ...
        aucs.append(roc_auc_score(y_event[te], prob))
        model = GatedMultiStream(1); torch_seed(); opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4); time_t = torch.tensor(y_time[tr], dtype=torch.float32); event_t = torch.tensor(y_event[tr], dtype=torch.float32)
        n = train_imgs[0].shape[0]; model.train()
        for _ in range(EPOCHS):
            perm = torch.randperm(n)
            for i in range(0, n, BATCH_SIZE):
...
def main() -> None:
    ...
    for omics in OMICS:
        ...
        rows.append({"model": f"Gated_{omics}", "metric": "roc_auc", "value": round(auc, 4)})
        rows.append({"model": f"Gated_{omics}", "metric": "c_index", "value": round(ci, 4)})
        print(f"{omics}: AUC={auc:.4f} C-index={ci:.4f}")
    auc, ci = run_multi([matrices[o] for o in OMICS], labels)
    rows.append({"model": "Gated_MultiStream", "metric": "roc_auc", "value": round(auc, 4)})
    rows.append({"model": "Gated_MultiStream", "metric": "c_index", "value": round(ci, 4)})
