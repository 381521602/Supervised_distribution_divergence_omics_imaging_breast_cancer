#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Compare JSD as pixel weight vs JSD vector concatenated after branches."""
...
OUT = DATA / "jsd_weight_variants_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 60
BATCH_SIZE = 64
...
class MultiStreamJSDConcat(nn.Module):
    def __init__(self, out_dim, jsd_dims):
        super().__init__()
        self.mrna = ConvBranch(SIZES["mRNA"], CHANNELS)
        self.cnv = ConvBranch(SIZES["CNV"], CHANNELS)
        self.mirna = ConvBranch(SIZES["miRNA"], CHANNELS)
    ...
    def forward(self, x1, x2, x3, jsd_vec):
        f = torch.cat([self.mrna(x1), self.cnv(x2), self.mirna(x3), jsd_vec], dim=1)
        return self.head(f)
...
def train_concat(X_train, X_test, y_train, y_test, out_dim, jsd_train, jsd_test):
    torch_seed()
    model = MultiStreamJSDConcat(out_dim, [jsd_train.shape[1]]).to(DEVICE)
    opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4)
    crit = nn.CrossEntropyLoss()
    Xt = [torch.tensor(x, dtype=torch.float32) for x in X_train]
    Xe = [torch.tensor(x, dtype=torch.float32) for x in X_test]
    Nt = torch.tensor(jsd_train, dtype=torch.float32)
    Ne = torch.tensor(jsd_test, dtype=torch.float32)
    yt = torch.tensor(y_train, dtype=torch.long)
    n = Xt[0].shape[0]; model.train()
    for _ in range(EPOCHS):
        perm = torch.randperm(n)
...
def main() -> None:
    ...
    for variant in ["base", "weight", "concat"]:
        ...
        for tr, te in cv.split(np.zeros(len(common)), y):
            train_imgs, test_imgs, jsd_train_parts, jsd_test_parts = [], [], [], []
            for block, omics in zip(X_blocks, OMICS):
                ...
                jsd_train_parts.append(norm)
                jsd_test_parts.append(norm)
                if variant == "weight":
            ...
            if variant == "concat":
                jsd_vec = np.concatenate(jsd_train_parts, axis=0)
                jsd_train = np.tile(jsd_vec, (len(tr), 1))
                jsd_test = np.tile(jsd_vec, (len(te), 1))
                pred = train_concat(train_imgs, test_imgs, y[tr], y[te], len(enc.classes_), jsd_train, jsd_test)
            else:
