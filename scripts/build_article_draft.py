...
def main():
    ...
    add_body(doc, "本研究提出一种基于组学数据图像化的乳腺癌多组学融合分析框架。首先对 TCGA-BRCA 的 mRNA、CNV、miRNA 数据进行样本对齐和特征筛选，采用新对称相对熵（NSRE）进行特征排序与加权，并将一维特征转换为二维灰度图；随后构建全尺寸卷积多流网络，并引入门控机制和 Mish 激活。结果表明，PAM50 分子分型中门控 NSRE 加权全尺寸卷积网络达到约 0.86 的准确率，生存预测中 mRNA 扩增特征后 Cox C-index 可达 0.64。关键基因挖掘识别出若干与乳腺癌相关的候选标志物，为图像化多组学融合提供了一种可解释的新方法。")

    add_heading(doc, "1. 引言", 1)
    add_body(doc, "乳腺癌具有高度分子异质性，多组学整合分析可揭示不同层次的分子机制。传统机器学习以向量形式输入高维组学特征，难以利用特征间的空间关系。本研究将组学特征图像化，并使用全尺寸卷积网络进行融合预测，同时引入 NSRE 信息熵特征排序和门控机制，提升分类与生存预测性能。")

    add_heading(doc, "2. 数据与方法", 1)
    add_body(doc, "数据来源于 TCGA-BRCA 的 GDC 与 UCSC Xena。共 1098 例乳腺癌，其中 1073 例同时具备 mRNA、miRNA 和基因级 CNV。特征筛选采用方差过滤、F 值、L1、NSRE 及其组合。图像化使用 NSRE 排序与加权，将特征填入方形灰度图；模型采用多流全尺寸卷积，并加入门控机制与 Mish 激活。全部实验使用 5 折分层交叉验证，随机种子为 42。")
    ...
    add_body(doc, "消融实验显示，全尺寸卷积优于 3×3 残差、多尺度、Inception、MobileNet、Grouped Conv 等结构；NSRE 像素加权将 PAM50 准确率从 0.813 提升至 0.860；门控机制进一步提升 Macro-F1 至 0.851。Mish 激活略优于 ReLU。")
    add_figure(doc, FIG / "fig4_adaptive_pipeline.png", "图 3. 自适应 NSRE 特征筛选与图像化流程")
