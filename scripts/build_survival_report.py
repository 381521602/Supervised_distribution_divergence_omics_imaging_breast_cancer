...
def main():
    ...
    add_body(doc,"本研究在 TCGA-BRCA 生存预测任务上，系统比较了低方差过滤、F 值、L1、NSRE 及其两两组合等特征提取方法，以及逻辑回归、随机森林、梯度提升、支持向量机、K 近邻和 Cox 比例风险模型。结果表明，mRNA 在 V_NSRE_F 方法下 Cox C-index 达到 0.6604；CNV 在 V_L1_NSRE 方法下 C-index 为 0.6124；miRNA 在 V_L1_NSRE 方法下 C-index 为 0.5915。生存二分类 AUC 方面，mRNA 使用 V_L1_F+LogisticRegression 达到 0.6586。")
    ...
    add_body(doc,"生存预测最终数据集使用 TCGA-BRCA 的 mRNA、CNV、miRNA 数据。特征提取方案为：mRNA 采用 V_NSRE_F 保留 200 特征，CNV 采用 V_L1_NSRE 保留 150 特征，miRNA 采用 V_L1_NSRE 保留 600 特征。样本量分别为 1073、1057、739。")
    table=doc.add_table(rows=1, cols=4)
    hdr=table.rows[0].cells
    for i,txt in enumerate(["组学","特征方法","特征数","样本数"]):
        hdr[i].text=txt; set_cell_shading(hdr[i],"F2F4F7")
    for row in [("mRNA","V_NSRE_F","200","1073"),("CNV","V_L1_NSRE","150","1057"),("miRNA","V_L1_NSRE","600","739")]:
    ...
    add_body(doc,"低方差过滤：Var(x_j)<τ 的特征被删除，τ=0。F 值：F=MS_between/MS_within，选择 F 最大的 k 个特征。L1：min_w (1/2)||Xw-y||^2 + λ||w||_1。NSRE：NSRE(p||q)=Σ p_i log2(2p_i/(p_i+q_i)) + Σ q_i log2(2q_i/(p_i+q_i))。")
    ...
    add_equation(doc, '<m:oMathPara xmlns:m="http://schemas.openxmlformats.org/officeDocument/2006/math"><m:oMath><m:r><m:t>NSRE(p||q)=Σ p_i log2(2p_i/(p_i+q_i)) + Σ q_i log2(2q_i/(p_i+q_i))</m:t></m:r></m:oMath></m:oMathPara>')
    ...
    add_body(doc,"生存二分类 AUC 最优组合：mRNA 使用 V_L1_F+LogisticRegression、800 特征时 AUC=0.6586；CNV 使用 V_L1_F+RandomForest、50 特征时 AUC=0.5559；miRNA 使用 V_NSRE+RandomForest、200 特征时 AUC=0.6046。")
    add_body(doc,"Cox C-index 最优组合：mRNA 使用 V_NSRE_F、200 特征时 C-index=0.6604；CNV 使用 V_L1_NSRE、150 特征时 C-index=0.6124；miRNA 使用 V_L1_NSRE、800 特征时 C-index=0.5915。")
    ...
    add_body(doc,"mRNA 在生存预测中表现最好，C-index 可达 0.66；CNV 和 miRNA 较弱。不同组学适合的特征方法和特征数量不同：mRNA 使用 NSRE 相关组合，CNV 和 miRNA 使用 L1_NSRE 组合。")
    ...
    for i,txt in enumerate(["组学","特征方法","特征数","C-index","选择理由"]):
        hdr[i].text=txt; set_cell_shading(hdr[i],"F2F4F7")
    rows=[
        ("mRNA","V_NSRE_F","200","0.6604","200 特征已最优，效率最高"),
        ("CNV","V_L1_NSRE","150","0.6124","150 为拐点，200 特征收益很小"),
        ("miRNA","V_L1_NSRE","600","0.5862","600 接近 800 的最优值，计算更高效"),
    ]
    for row in rows:
        cells=table.add_row().cells
        for i,txt in enumerate(row): cells[i].text=txt
    ...
    add_body(doc,"本研究为乳腺癌生存预测提供了系统的特征提取和模型选择依据。综合考虑预测精度与计算效率，推荐 mRNA 采用 V_NSRE_F、200 特征；CNV 采用 V_L1_NSRE、150 特征；miRNA 采用 V_L1_NSRE、600 特征。mRNA 是生存预测的主要组学。")
