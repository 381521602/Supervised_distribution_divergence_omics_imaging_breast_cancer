#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build detailed feature-scaling report with formulas and error tables."""
...
OUT = ROOT / "data" / "特征扩增实验报告_详细版.docx"
...
def main():
    ...
    for omics in ["mRNA","CNV","miRNA"]:
        ...
        for row in rows:
            cells=add_table.add_row().cells; cells[0].text=row[0]; cells[1].text=row[1]
        set_table_widths(add_table,[1600,3000])
