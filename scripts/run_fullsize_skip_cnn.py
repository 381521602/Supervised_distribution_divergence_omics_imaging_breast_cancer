#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Full-size conv with raw-vector skip connection."""
...
OUT = DATA / "fullsize_skip_cnn_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 80
BATCH_SIZE = 64
...
class SkipNet(nn.Module):
    ...
    def forward(self, x_mrna_img, x_cnv_img, x_mirna_img, raw_blocks):
        f1 = self.mrna(x_mrna_img)
        f2 = self.cnv(x_cnv_img)
        f3 = self.mirna(x_mirna_img)
        raw = torch.cat(raw_blocks, dim=1)
        return self.head(torch.cat([f1, f2, f3, raw], dim=1))
...
def run_fold(train_imgs, test_imgs, raw_train, raw_test, y_train, y_test, out_dim):
    torch_seed()
    model = SkipNet(out_dim, [x.shape[1] for x in raw_train]).to(DEVICE)
    ...
    R_train = [torch.tensor(x, dtype=torch.float32) for x in raw_train]
    R_test = [torch.tensor(x, dtype=torch.float32) for x in raw_test]
    ...
    for _ in range(EPOCHS):
        ...
        for i in range(0, n, BATCH_SIZE):
            ...
            out = model(
                ...
                X_train[1][idx],
                X_train[2][idx],
                [R_train[j][idx] for j in range(len(R_train))],
            )
    ...
    with torch.no_grad():
        logits = model(
            ...
            X_test[1],
            X_test[2],
            [R_test[j] for j in range(len(R_test))],
        )
...
def main() -> None:
    ...
    for train_idx, test_idx in cv.split(np.zeros(len(common)), y):
        train_imgs, test_imgs, raw_train, raw_test = [], [], [], []
    ...
    rows = [
        {"model": "FullSizeConv_SkipRawVector", "metric": "accuracy", "value": round(float(np.mean(accs)), 4)},
        {"model": "FullSizeConv_SkipRawVector", "metric": "macro_f1", "value": round(float(np.mean(f1s)), 4)},
    ]
