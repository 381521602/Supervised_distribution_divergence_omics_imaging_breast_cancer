#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""MobileNet-style depthwise-separable full-size CNN."""
...
OUT = DATA / "mobilenet_fullsize_cnn_results.tsv"
RANDOM_STATE = 42
K_FOLDS = 5
EPOCHS = 60
BATCH_SIZE = 64
...
class MobileBranch(nn.Module):
    def __init__(self, size, channels):
        super().__init__()
        self.depthwise = nn.Conv2d(1, 1, kernel_size=(size, size), groups=1)
        self.pointwise = nn.Conv2d(1, channels, 1)
...
class MobileMultiStream(nn.Module):
    def __init__(self, out_dim):
        super().__init__()
        self.mrna = MobileBranch(SIZES["mRNA"], CHANNELS)
        self.cnv = MobileBranch(SIZES["CNV"], CHANNELS)
        self.mirna = MobileBranch(SIZES["miRNA"], CHANNELS)
...
def main() -> None:
    ...
    for tr, te in cv.split(np.zeros(len(common)), y):
        ...
        model = MobileMultiStream(len(enc.classes_)); torch_seed(); opt = torch.optim.Adam(model.parameters(), lr=1e-3, weight_decay=1e-4); crit = nn.CrossEntropyLoss(); yt = torch.tensor(y[tr], dtype=torch.long)
    ...
    rows = [
        {"model": "MobileNet_NSRE_Weighted_FullSizeCNN", "metric": "accuracy", "value": round(float(np.mean(accs)), 4)},
        {"model": "MobileNet_NSRE_Weighted_FullSizeCNN", "metric": "macro_f1", "value": round(float(np.mean(f1s)), 4)},
    ]
