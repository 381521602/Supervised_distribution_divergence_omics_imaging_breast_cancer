...
def make_selector(fs, k):
    ...
    if fs == "V_NSRE":
        return Pipeline([("var", var), ("nsre", NSRESelector(prefilter_k=max(k*5,1000), k=k))])
    if fs == "V_F_L1":
    ...
    if fs == "V_L1_NSRE":
        return Pipeline([("var", var), ("l1", SelectFromModel(LinearSVC(penalty="l1", dual=False, C=0.1, max_iter=5000, random_state=RANDOM_STATE), max_features=min(k*5,5000), threshold=-np.inf)), ("nsre", NSRESelector(prefilter_k=min(k*5,5000), k=k))])
    if fs == "V_NSRE_F":
        return Pipeline([("var", var), ("nsre", NSRESelector(prefilter_k=max(k*5,1000), k=min(k*5,5000))), ("f", SelectKBest(score_func=f_classif, k=k))])
    if fs == "V_NSRE_L1":
        return Pipeline([("var", var), ("nsre", NSRESelector(prefilter_k=max(k*5,1000), k=min(k*5,5000))), ("l1", SelectFromModel(LinearSVC(penalty="l1", dual=False, C=0.1, max_iter=5000, random_state=RANDOM_STATE), max_features=k, threshold=-np.inf))])
    raise ValueError(fs)
