#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""English technical-route figure v19: no title, compact cards with icons."""
...
OUT_PNG = ROOT / "data/paper_figures/fig1_route_v19.png"
OUT_SVG = ROOT / "data/paper_figures/fig1_route_v19.svg"
...
def draw_icon(ax, kind, x, y, color):
    ...
    if kind == "db":
        ...
        ax.plot([x + r[i] * __import__("math").cos(t[i]) for i in range(100)], [y + r[i] * __import__("math").sin(t[i]) for i in range(100)], color=color, lw=1.6, zorder=4)
    elif kind == "net":
...
def main():
    ...
    for idx, (no, title, color, modules, face, edge) in enumerate(phases):
        ...
        ax.text(x + 0.36, box_y + box_h - header_h / 2 - 0.07, no, ha="center", va="center", fontsize=12, fontweight="bold", color="white", zorder=4)
        ax.text(x + 0.64, box_y + box_h - header_h / 2 - 0.07, title.replace("\n", " "), ha="left", va="center", fontsize=8.8, fontweight="bold", color="white", zorder=4)
        ...
        for j, (icon_kind, item_title, item_desc) in enumerate(modules):
            ...
            ax.text(x + 1.02, card_y + card_h - 0.38, item_title, ha="left", va="center", fontsize=8.7, fontweight="bold", color=color, zorder=4)
            ax.text(x + 1.02, card_y + 0.34, item_desc, ha="left", va="center", fontsize=7.7, color="#4B5B6B", zorder=4)
    ...
    ax.text(2.40, band_y + band_h / 2, "Interpretability", ha="center", va="center", fontsize=9.2, fontweight="bold", color="#9F3F3F", zorder=4)
    ...
    for x, label in pills:
        ...
        ax.text(x + 1.10, band_y + band_h / 2, label, ha="center", va="center", fontsize=8.0, fontweight="bold", color="#A23E3E", zorder=4)
