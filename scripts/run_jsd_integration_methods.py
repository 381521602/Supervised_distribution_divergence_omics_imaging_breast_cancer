#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Compare 6 JSD integration methods for mRNA images."""
...
OUT=ROOT/"data/jsd_integration_methods_results.tsv"
...
class MethodNet(nn.Module):
    def __init__(self,method,size,out_dim,jsd_dim):
        super().__init__()
        self.method=method
        if method=="1_twochannel":
            self.block=ConvBlock(2,32,size); feat=32
        elif method=="2_concat_vector":
            self.block=ConvBlock(1,32,size); feat=32+jsd_dim
    ...
    def forward(self,x,jsd=None,jsd_vec=None):
        if self.method=="1_twochannel":
            x=torch.cat([x,jsd],dim=1)
            f=self.block(x)
        elif self.method=="2_concat_vector":
            f=self.block(x); f=torch.cat([f,jsd_vec],dim=1)
        elif self.method=="3_spatial_attention":
            f=self.block(x)
        elif self.method=="5_learnable_fusion":
            x=x*self.alpha + jsd*(1-self.alpha)
            f=self.block(x)
        else:
            f=self.block(x)
        return self.head(f)
...
def prepare(task,omics,size):
    ...
    if jsd.max()>jsd.min(): w=(jsd-jsd.min())/(jsd.max()-jsd.min()+1e-9)
    else: w=np.zeros_like(jsd)
    jsd_grid=np.zeros((size,size),dtype=np.float32)
    for pos,val in zip(spiral_order(size),w): jsd_grid[pos]=val
    jsd_img=np.repeat(jsd_grid[np.newaxis,np.newaxis,:,:],len(imgs),axis=0)
    jsd_vec=np.tile(w,(len(imgs),1))
    return imgs.astype(np.float32), jsd_img.astype(np.float32), jsd_vec.astype(np.float32)


def train(method,Xtr,jsd_img_tr,jsd_vec_tr,ytr,Xte,jsd_img_te,jsd_vec_te,size,dout,binary=False):
    torch.manual_seed(RANDOM_STATE); np.random.seed(RANDOM_STATE)
    model=MethodNet(method,size,dout,jsd_vec_tr.shape[1])
    ...
    Xt=torch.tensor(Xtr,dtype=torch.float32); Nimg=torch.tensor(jsd_img_tr,dtype=torch.float32); Nvec=torch.tensor(jsd_vec_tr,dtype=torch.float32)
    n=Xt.shape[0]
    ...
    model.eval()
    with torch.no_grad():
        out=model(torch.tensor(Xte,dtype=torch.float32),torch.tensor(jsd_img_te,dtype=torch.float32),torch.tensor(jsd_vec_te,dtype=torch.float32))
    return torch.sigmoid(out).numpy().ravel() if binary else out.argmax(dim=1).numpy()


def eval_method(method,task,omics,size):
    imgs,jsd_img,jsd_vec=prepare(task,omics,size)
    if task=="PAM50":
        ...
        for tr,te in cv.split(np.zeros(len(y)),y):
            pred=train(method,imgs[tr],jsd_img[tr],jsd_vec[tr],y[tr],imgs[te],jsd_img[te],jsd_vec[te],size,len(enc.classes_))
        ...
        for tr,te in cv.split(np.zeros(len(y_event)),y_event):
            prob=train(method,imgs[tr],jsd_img[tr],jsd_vec[tr],y_event[tr],imgs[te],jsd_img[te],jsd_vec[te],size,1,binary=True)
